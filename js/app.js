...
(function () {
  ...
  function renderTimeline(filter = "all") {
    ...
    if (filter === "tokyo") days = days.filter(d => d.cityEn === "Tokyo");
      ...
      return `
        <div class="day-card ${isToday ? "today" : ""} ${isPast ? "past" : ""}" data-date="${day.date}">
          <div class="day-card-header">
            ...
            <div class="day-card-weather">🌤️</div>
          </div>
  ...
  function openDay(dateStr) {
    ...
    let html = `
      <div class="day-header">
        ...
        ${hotel ? `<div class="meta mt-8">🏨 ${hotel.nameHe} (${hotel.nameEn})</div>` : ""}
  ...
  function setupCurrency() {
    const rate = data.trip.currencyRate;
  ...
  function renderTools() {
    ...
    hotEl.innerHTML = data.hotels.map(h => `
      <div class="hotel-card">
        ...
        ${h.bookingUrl ? `<a href="${h.bookingUrl}" target="_blank" rel="noopener">קישור ל-Booking</a>` : ""}
      </div>
    `).join("");
